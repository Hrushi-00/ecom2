...
import "./Cart.css"; // Ensure this file is imported correctly
...
const ProductSummaryPage = ({ items }) => {
  ...
  const [quantity, setQuantity] = useState(items.map(item => item.quantity || 1)); // Initialize quantities for all items

  // Function to calculate total price for all items
  const calculateTotalPrice = () => {
    return items.reduce((total, item, index) => {
      const price = parseFloat(item.price); // Ensure price is valid number
      return total + price * quantity[index]; // Add price * quantity for each item
    }, 0).toFixed(2);
  };
              ...
              <div className="project-card">
                <div className="product-cart-main">
                  ...
                  <div className="quantity-controls">
                    ...
                    <span style={{position:"relative", bottom:-1, padding:"7px", marginLeft:"8px"}}>{quantity[index]}</span>
                    <button  className="btn1" onClick={() => {
                      const updatedQuantity = [...quantity];
                      updatedQuantity[index] += 1;
                      setQuantity(updatedQuantity);
